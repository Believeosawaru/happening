...
async function searchUsers() {
    ...
    try {
        const response = await fetch(`https://happening-api.onrender.com/api/v1/user/group/search/email-users/${groupId}?query=${encodeURIComponent(query)}`, {
            method: "GET",
            headers: {
...
async function addMemberToGroup(userId) {
    try {
        const response = await fetch(`https://happening-api.onrender.com/api/v1/user/group/${groupId}/send-invite`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
